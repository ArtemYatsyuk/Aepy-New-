...
import { Play, Pause, Clock, Heart, Share2, Download, CheckCircle2 } from 'lucide-react';
...
export default function AlbumView() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      setSongs(allSongs.filter((s: any) => s.albumId === id));
    }
    fetchData();
  }, [id]);
  ...
  const isAlbumPlaying = currentSong && songs.some(s => s.id === currentSong.id);
  ...
  return (
    <div className="pb-8 relative">
      ...
      <div className="h-64 bg-gradient-to-b from-gray-700 to-[#121212] flex items-end p-8 gap-6">
        <img src={album.coverUrl} alt={album.title} className="w-48 h-48 shadow-2xl rounded" />
        <div className="flex flex-col gap-2 text-white">
          <span className="text-sm font-bold uppercase">Album</span>
          <h1 className="text-5xl font-black tracking-tighter">{album.title}</h1>
          <div className="flex items-center gap-2 text-sm mt-2">
            {artist && (
              <>
                <img src={artist.imageUrl} alt={artist.name} className="w-6 h-6 rounded-full" />
                <Link to={`/artist/${artist.id}`} className="font-bold hover:underline">{artist.name}</Link>
                <span className="text-[#B3B3B3]">•</span>
              </>
            )}
            <span className="text-[#B3B3B3]">{album.year} • {songs.length} songs,</span>
      ...
      <div className="px-8 py-6">
        ...
        <div className="grid grid-cols-[16px_minmax(120px,1fr)_minmax(120px,1fr)_minmax(120px,1fr)] gap-4 px-4 py-2 text-[#B3B3B3] text-sm border-b border-[#282828] mb-4">
          <div>#</div>
          <div>Title</div>
          <div></div>
        ...
        <div className="flex flex-col">
          {songs.map((song, index) => {
            ...
            return (
              <div 
                key={song.id}
                className={`grid grid-cols-[16px_minmax(120px,1fr)_minmax(120px,1fr)_minmax(120px,1fr)] gap-4 px-4 py-2 rounded-md hover:bg-[#ffffff1a] group items-center text-sm ${isCurrent ? 'text-[#1DB954]' : 'text-[#B3B3B3]'}`}
              >
                ...
                <div className="flex flex-col cursor-pointer" onClick={() => setQueue(songs, index)}>
                  <span className={`font-medium truncate ${isCurrent ? 'text-[#1DB954]' : 'text-white'}`}>{song.title}</span>
                  <span className="truncate group-hover:text-white transition-colors">{song.artist}</span>
                </div>
                <div></div>
